...
def edit_manager():
    if request.method == "POST":
        ...
        username = request.form["username"]
        boss_email = request.form["boss-email"]
        manager = Manager.read_one(Manager, manager_id)
        if User.username_taken(User, username, manager.user.username):
            flash(f"The username '{username}' is already taken! Please try another.", "danger")
            return redirect(url_for('manager.edit_manager'))
        Manager.update(current_user.manager, first_name, last_name, email, username, boss_email)
